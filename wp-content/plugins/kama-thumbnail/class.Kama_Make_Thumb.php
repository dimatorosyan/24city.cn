...
class Kama_Make_Thumb {
	...
	/**
	 * Makes tumb file name.
	 *
	 * @param string $srcpath_md5  MD5 hash of original src path: `parse_url( $this->src, PHP_URL_PATH )`.
	 * @param int    $width        Thumb width.
	 * @param int    $height       Thumb height.
	 * @param string $_suffix      Some additional string like: _notcrop, _notrise, right...
	 * @param string $ext          Image extension.
	 * @param string $srcpath      Original path of src for pass it to hook.
	 *
	 * @return mixed|void
	 */
	static function _make_file_name( $srcpath_md5, $width, $height, $_suffix, $ext, $srcpath ){

		$file_name = substr( $srcpath_md5, -15 ) . "_{$width}x{$height}$_suffix.$ext";

		return apply_filters( 'kama_thumb_make_file_name', $file_name, $width, $height, $_suffix, $ext, $srcpath );
	}
	...
	function do_thumbnail(){
		...
		// запрос отправил этот плагин, завершаем чтобы избежать рекурсии
		...
		if( apply_filters_ref_array( 'pre_do_thumbnail_src', [ '', & $this ] ) )
			return null;
		...
		$file_name        = self::_make_file_name( $srcpath_md5, $this->width, $this->height, $_suffix, $ext, $srcpath );
		...
		// CACHE - return the thumbnail if it's cache exists
		if( ! $this->debug ){
		...
		// NOT CACHE - create thumb and cache it
		...
		// Create thumb
		$forse_imagick = 'Imagick' === $this->args['force_lib'];
		$forse_gd      = 'GD'      === $this->args['force_lib'];

		// Imagick
		if( (extension_loaded('imagick') && ! $forse_gd) || $forse_imagick ){

			if( ! $this->make_thumbnail_Imagick( $img_str ) )
				$this->thumb_url = '';
		}
		// GD
		elseif( extension_loaded('gd') || $forse_gd ){
	...
	protected function make_thumbnail_GD( $img_string ){
		...
		if( 'png' === $format ){
			...
			imagepng( $thumb, $this->thumb_path );
		}
		elseif( 'gif' === $format ){
			imagegif( $thumb, $this->thumb_path );
		}
		else {
			imagejpeg( $thumb, $this->thumb_path, $this->quality );
		}
		chmod( $this->thumb_path, 0644 );
		imagedestroy( $image );
		imagedestroy( $thumb );
	...
	protected function set_args( $args = [], $src = 'notset' ){
		...
		$def_args = apply_filters( 'kama_thumb_default_args', [
			...
			'force_lib' => '', // GD, Imagick - force lib use
		] );
	...
	function img(){
		...
		$attr[] = 'width="'. intval( $this->width ) .'"';
		$attr[] = 'height="'. intval( $this->height ) .'"';
